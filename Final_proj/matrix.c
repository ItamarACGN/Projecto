...
/* Left Multiplication: Diag*A where Diag is a diagonal matrix, O(n^2) time */
Matrix* diagonal_multiply_matrix(const Matrix *Diag, const Matrix *A) {
    ...
    for (i = 0; i < A->rows; i++) {
        diag_val = Diag->data[i * Diag->cols + i]; /*D[i][i]*/
        for (j = 0; j < A->cols; j++) {
...
/* Right Multiplication: A*Diag where Diag is a diagonal matrix, O(n^2) time */
Matrix* matrix_multiply_diagonal(const Matrix *A, const Matrix *Diag) {
    ...
    for (j = 0; j < A->cols; j++) {
        diag_val = Diag->data[j * Diag->cols + j]; /* D[j][j] */
        for (i = 0; i < A->rows; i++) {
