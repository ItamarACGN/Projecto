...
static PyObject* kmeans_capi(PyObject *self, PyObject *args)
{
    ...
    int N, D, K, max_iters;
    double epsilon;
    if(!PyArg_ParseTuple(args, "iiiiOOd", &N, &D, &K, &max_iters, &data, &centroids, &epsilon)) {
        return NULL;
    }
    struct Vector* data_vector = pyList_to_vectors(data);
    struct Vector* centroids_vector = pyList_to_vectors(centroids);
    struct Vector* result_centroids = fit(data_vector, centroids_vector, N, D, K, max_iters, epsilon);
